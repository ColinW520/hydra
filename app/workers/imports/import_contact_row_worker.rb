class Imports::ImportContactRowWorker
  ...
  def perform(row, import_id)
    ...
    contact = Contact.where(organization_id: the_organization.id, mobile_phone: PhonyRails.normalize_number(row[:phone])).first_or_initialize

    contact.organization_id = the_import.organization_id
    contact.first_name = row[:first_name]
    contact.last_name = row[:last_name]
    contact.mobile_phone = PhonyRails.normalize_number(row[:phone])
    ...
    if contact.persisted?
      ...
      contact.tag_list = tags_array.join(', ')
    else
